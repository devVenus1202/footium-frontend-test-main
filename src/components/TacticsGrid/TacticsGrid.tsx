...
const swapPlayers = (fromPlayer: Player, toPlayer: Player | any, players: Team, setPlayers: any) => {
  let pls = {...players};
  let fromIndex = -1;
  let toIndex = -1;

  console.log("fromPlayer", pls)
  console.log("fromPlayer----", fromPlayer)
  console.log("toPlayer", toPlayer)
  if (fromPlayer.isSub || toPlayer.isSub) {
    console.log("first", fromPlayer)
...
const handleDrop = (data: any, players: any, setPlayers: any) => (event: any) => {
    ...
    <div className={mc(classes.container)}>
      <div className={mc(classes.pitchBackground)}>
        <div className={mc( classes.pitch )} >
            {players.firstEleven.map((player, index) => (
              <PlayerCard
                player={player}
                key={player.id}
                draggable
                ...
                style={{gridRowStart:position[index][0] + 1, gridColumnStart:position[index][1] + 1}}
              />
            ))}
        </div>
      </div>
